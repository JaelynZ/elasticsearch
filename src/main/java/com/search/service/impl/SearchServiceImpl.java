...
import com.search.entity.Suggestion;
...
public class SearchServiceImpl implements SearchService {
    ...
    @Override
    public AutoSuggestionResp autoSuggestion(String hintKey, Integer count){
        AutoSuggestionResp resp = new AutoSuggestionResp();
        Integer from = 0;
        Integer size = count;
        hintKey = stconvert(hintKey);
        //搜索词
        QueryBuilder queryBuilder = QueryBuilders.multiMatchQuery(hintKey, "title.cn");
        SearchResponse searchResponse = client.getConnection().prepareSearch(esIndex)
                .setTypes(esType)
                .setSearchType(SearchType.DFS_QUERY_THEN_FETCH)
                .setQuery(queryBuilder)
                .highlighter(new HighlightBuilder().field("title.cn"))
                .setFrom(from).setSize(size).setExplain(true)
                .get();
        SearchHits hits = searchResponse.getHits();
        List<Suggestion> suggestionResult = Lists.newArrayList();
        for (SearchHit hit : hits) {
            ...
            if(highlightFieldMap != null){
                Text[] text = highlightFieldMap.get("title.cn").getFragments();
                String highLight = "";
                for (Text str : text) {
                    highLight = str.string();
                }
                suggestion.setHighLight(highLight);
            }
            suggestion.setHintWord(result.get("title").toString());
            suggestionResult.add(suggestion);
        }
        resp.setData(suggestionResult);
     ...
     * @return
     */
    public String stconvert(String hintKey){
        if(StringUtils.isEmpty(hintKey)){
            return "";
        }

        //简繁转换stconvert
        AnalyzeRequestBuilder stconvertRequest = new AnalyzeRequestBuilder(client.getConnection(),
                AnalyzeAction.INSTANCE, "stconvert", hintKey)
                .setTokenizer("keyword")
                .addCharFilter("tsconvert");
        List<AnalyzeResponse.AnalyzeToken> stconvertTokenList = stconvertRequest.execute().actionGet().getTokens();
        List<String> stconvertTermList = Lists.newArrayList();
        stconvertTokenList.forEach(ikToken -> stconvertTermList.add(ikToken.getTerm()));
        return stconvertTermList.get(0);
    }

    @Override
    public SearchResp search(String hintKey, Integer pageNo, Integer pageSize) {
...
/*        List<String> phraseList = Lists.newArrayList();
        ...
        //ik + stemmer分词
        AnalyzeRequestBuilder ikRequest = new AnalyzeRequestBuilder(client.getConnection(),
                AnalyzeAction.INSTANCE, "ik_stemmer", hintKey)
                .setAnalyzer("ik_stemmer");
        List<AnalyzeResponse.AnalyzeToken> ikTokenList = ikRequest.execute().actionGet().getTokens();
        ikTokenList.forEach(ikToken -> phraseList.add(ikToken.getTerm()));

        //pinyin分词
        AnalyzeRequestBuilder pinyinRequest = new AnalyzeRequestBuilder(client.getConnection(),
                AnalyzeAction.INSTANCE, "index", hintKey)
                .setTokenizer("pinyin");
        List<AnalyzeResponse.AnalyzeToken> pinyinTokenList = pinyinRequest.execute().actionGet().getTokens();
        pinyinTokenList.forEach(ikToken -> phraseList.add(ikToken.getTerm()));*/

        //搜索词
        /*QueryBuilder queryBuilder = QueryBuilders.boolQuery()
                .should(QueryBuilders.matchQuery("title", "中长款"))
                .should(QueryBuilders.matchQuery("title","针织"))
                .should(QueryBuilders.matchQuery("title","连衣裙"));*/

        QueryBuilder queryBuilder = QueryBuilders.multiMatchQuery(hintKey, "title", "title.cn", "title.en","title.pinyin");

        /*BoolQueryBuilder boolQueryBuilder = new BoolQueryBuilder();
        phraseList.stream().forEach(phrase -> {
            boolQueryBuilder.should(QueryBuilders.matchQuery("title", phrase));
        });
        ...
        SearchResponse searchResponse = client.getConnection().prepareSearch(esIndex)
                ...
                .setSearchType(SearchType.DFS_QUERY_THEN_FETCH)
                .setQuery(queryBuilder)
//                .setPostFilter(QueryBuilders.rangeQuery("attention").from(15).to(200))
                .addSort("_score",SortOrder.DESC)
                .addSort("isRecommend",SortOrder.DESC)
                .addSort("attention",SortOrder.DESC)
                .addSort("createTime",SortOrder.DESC)
                .addSort("id",SortOrder.DESC)
                .highlighter(new HighlightBuilder().field("title").field("title.cn").field("title.en").field("title.pinyin"))
                .setFrom(from).setSize(size).setExplain(true)
        ...
        for (SearchHit hit : hits) {
            ...
            if(highlightFieldMap != null){
                Text[] text = null;
                if(highlightFieldMap.get("title.pinyin") != null){
                    text = highlightFieldMap.get("title.pinyin").getFragments();
                }else if(highlightFieldMap.get("title.cn") != null){
                    text = highlightFieldMap.get("title.cn").getFragments();
                }else if(highlightFieldMap.get("title.en") != null){
                    text = highlightFieldMap.get("title.en").getFragments();
                }else if(highlightFieldMap.get("title") != null){
                    text = highlightFieldMap.get("title").getFragments();
                }
                String highLight = "";
                for (Text str : text) {
                    highLight = str.string();
